...
public class PlayCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event) 
    {
        ...
        String loadQuery = SoundCloudUtil.isSoundCloudHttpUrl(args) ? SoundCloudUtil.normalizePlaybackUrl(args) : args;
        if(SpotifyUtil.playIfSpotifyUrl(bot, event, loadQuery, loadingEmoji, false,
                (m, ev, q) -> bot.getPlayerManager().loadItemOrdered(ev.getGuild(), q, new ResultHandler(m, ev, false))))
            return;
        event.reply(loadingEmoji+" Loading... `["+loadQuery+"]`", m -> bot.getPlayerManager().loadItemOrdered(event.getGuild(), loadQuery, new ResultHandler(m,event,false)));
    }
